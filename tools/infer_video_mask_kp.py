...
def main():
    ...
    ## add keypoint model
    cfg.MODEL.NUM_CLASSES = 2
    cfg_from_file('configs/baselines/e2e_keypoint_rcnn_R-50-FPN_s1x.yaml')
    cfg.MODEL.LOAD_IMAGENET_PRETRAINED_WEIGHTS = False  # Don't need to load imagenet pretrained weights
    assert_and_infer_cfg()
    keypoint_maskRCNN = Generalized_RCNN()
    if args.cuda:
        keypoint_maskRCNN.cuda()
    load_detectron_weight(keypoint_maskRCNN, '/home/work/liupeng11/code/Detectron.pytorch/models/keypoint_R-50-FPN.pkl')
    keypoint_maskRCNN = mynn.DataParallel(keypoint_maskRCNN, cpu_keywords=['im_info', 'roidb'], minibatch=True, device_ids=[0])  # only support single GPU
    keypoint_maskRCNN.eval()
    ...
    while True:
        ...
        if len(frame_list) == batch_size:
            for i, frame in enumerate(frame_list):
                ...
                cls_boxes, cls_segms, _ = im_detect_all(maskRCNN, frame, timers=timers)
                timers1 = defaultdict(Timer)
                _, _, cls_keyps = im_detect_all(keypoint_maskRCNN, frame, timers=timers1)
